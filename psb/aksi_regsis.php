...
if ($_POST['captcha'] == $_SESSION['captcha']['code']) {
	...
	/*
		...
		*/ else {
		...
		$password_baru = substr(md5(uniqid(rand(), 1)), 3, 10);
		$tujuan = $email;
		$subjek = "PSB Online SMK XYZ - Kode aktivasi dan password login calon siswa.";
		$link = "http://localhost/psbnew/aktivasi.php?code=$kode_aktivasi"; /*-- jika sudah hosting, ubah dengan link URL website --*/
		$pesan = "Selamat, Anda terdaftar sebagai calon siswa SMK XYZ. Klik link tautan berikut $link untuk aktivasi akun calon siswa. No. pendaftaran Anda adalah $nextRegNo dan password anda adalah $password_baru. Silahkan login menggunakan no. pendaftaran dan password tersebut.";
